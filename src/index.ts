      ...
      try {
        ...
        const filter = parsedSitemapUrl
          .filter((sitemapUrl: SitemapUrl) => {
            ...
            const containsDisallowedKeywords = disallowedKeywords.some(
              (keyword) => {
                return sitemapUrl.loc[0].includes(keyword);
              }
            );
            if (
              sitemapUrl.loc[0].includes("coingape") &&
